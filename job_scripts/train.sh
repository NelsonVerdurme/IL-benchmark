...
python genrobo3d/train/train_simple_policy.py \
    --exp-config genrobo3d/configs/rlbench/modified_ptv3.yaml \
    output_dir ${output_dir} \
    TRAIN.num_epochs null TRAIN.num_train_steps 300000 \
    TRAIN.log_steps 1000 TRAIN.save_steps 10000 TRAIN.val_steps 2 \
    TRAIN.train_batch_size 4 TRAIN.val_batch_size 4 \
    VAL_DATASET.use_val True \
    TRAIN_DATASET.data_dir ${HOME}/data/train/keysteps_bbox_pcd/voxel1cm \
    TRAIN_DATASET.instr_embed_file ${HOME}/data/train/keysteps_bbox_pcd/instr_embeds_clip.npy \
    ...
    VAL_DATASET.data_dir ${HOME}/data/train/keysteps_bbox_pcd/voxel1cm \
    VAL_DATASET.instr_embed_file ${HOME}/data/train/keysteps_bbox_pcd/instr_embeds_clip.npy \
    ...
    VAL_DATASET.sample_points_by_distance False \
    VAL_DATASET.taskvar_file assets/taskvars_peract.json \
    VAL_DATASET.taskvar_instr_file assets/taskvars_instructions_peract.json \
    VAL_DATASET.include_last_step False \
    VAL_DATASET.use_height True \
    VAL_DATASET.augment_pc False \
    VAL_DATASET.aug_max_rot 0 \
