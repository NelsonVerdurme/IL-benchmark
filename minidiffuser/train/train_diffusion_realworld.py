...
from minidiffuser.train.datasets.realworld_dataset import (
    RealworldDataset, base_collate_fn, ptv3_collate_fn
)
...
DATASET_FACTORY = {
    'DP': (RealworldDataset, ptv3_collate_fn),
}
...
@hydra.main(config_path="./", config_name="diffusion_realworld")
def hydra_main(config: DictConfig):
    if config.wandb_enable:
        ...
        wandb.init(project='real-diff', name=config.wandb_name + f"{time_id}_{str(uuid.uuid4())[:8]}", config=OmegaConf.to_container(config, resolve=True))
        main(config)
        wandb.finish()
